...
from tilia.ui.timelines.audiowave.constants import FRAMES_PER_PEAK_OPTIONS
...
class TestSettings:
    ...
    def test_frames_per_peak_setting_triggers_refresh(self, audiowave_tlui):
        # Establish a known baseline so the change below is unambiguous —
        # without this the test is order-dependent (settings persist across
        # tests in the module).
        settings.set("audiowave_timeline", "frames_per_peak", 128)
        audiowave_tlui._last_frames_per_peak = 128
    ...
    def test_color_change_does_not_trigger_refresh(self, audiowave_tlui):
        # Color is read by paint() on every call; full re-extraction is
        # only warranted when frames_per_peak changes.
        refresh_calls = []
        audiowave_tlui.timeline.refresh = lambda: refresh_calls.append(1)
        settings.set("audiowave_timeline", "default_color", "#FF00FF")
        post(Post.SETTINGS_UPDATED, ["audiowave_timeline"])
