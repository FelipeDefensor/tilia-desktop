...
class AudioWaveTimeline(Timeline):
    ...
    def deserialize_components(self, components: dict[int, dict[str, Any]]) -> None:
        # Pre-LOD-pyramid .tla files (≤ 0.6.0) stored a fixed-resolution
        # AmplitudeBar per bucket with start/end/amplitude fields. The
        # current Waveform component has none of those — we detect the
